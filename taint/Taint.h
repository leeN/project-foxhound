 ...
 *
 * These could also be shared between taint nodes, but it's most likely not
 * worth the effort.
 */
...
class TaintNode {
 ...
 private:
  ...
  // A node takes care of correctly addref()ing and release()ing its parent
  // node.
  TaintNode* parent_;
 ...
 * never merged into a new one, or, said differently, a node always has at most
 * one incoming edge. This might change in the future though and would be
 * required to correctly track the outcome of binary operations on primitive
 * types.
 */
class TaintFlow {
 private:
...
/*
 * This class represents a range of tainted bytes that share the same taint
 * flow.
 *
 ...
 *
 * This is the main data structure to describe taint information for string
 * types. It holds a set of taint ranges, each with a different taint flow. A
 * string is tainted if its StringTaint instance contains a least one taint
 * range.
 *
 * For example, the following piece of JavaScript code:
 *    var a = taint('abc');
 ...
 *    var x = a + b + c;
 *
 * will result in |x|'s StringTaint instance to contain two taint ranges: one
 * from [0, 3) and one from [6, 9). Both will have different taint flows
 ...
 *
 * For a more convenient version which deletes its own ranges, use
 * SafeStringTaint.
 */
class StringTaint {
 public:
...
/*
 * A version of StringTaint which deletes the taint range pointer when
 * destroyed.
 */
...
class TaintableString {
 ...
 protected:
  // Protected so child classes can do offset assertions (see
  // js/src/vm/String.h).
 ...
 *
 * Simpler than the StringTaint, for cases where a vector of TaintFlows is
 * sufficient
 *
 */
class TaintList {
